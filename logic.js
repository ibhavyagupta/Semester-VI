...
function showProducts() {
  const productsTab = document.getElementById("wr-productsTab");
...
window.onresize = function() {
  if (window.innerWidth > 767) {
    const allCards = document.querySelectorAll(".wr-ptql-card");
    ...
    const activeCards = document.querySelectorAll(".wr-ptql-card.active");
...
// --- Keyboard navigation enhancement (NO eventListeners used) ---
cards.forEach((header, index) => {
  header.tabIndex = 0; // ensure focusable

  header.onkeydown = function(e) {
    // Check for Tab key
    if (e.key === "Tab" && window.innerWidth <= 767) {
      const card = header.parentElement;
      const links = card.querySelector(".wr-ptql-links");
      const isActive = card.classList.contains("active");

      // If collapsed, skip its links
